...
public:
    ...
    virtual void sendToken(NetworkPort* src, const std::vector<Location>& dsts, Word word, CbIdx cbidx) = 0;
...
public:
    ...
    void sendToken(NetworkPort* src, const std::vector<Location>& dsts, Word word, CbIdx cbidx);
  ...
  private:
    class BusEvent : public CgraEvent {
      public:
        BusEvent(Cycles _timestamp, Cgra* _cgra, BusNetwork * _network, NetworkPort* _src, const std::vector<Location>& _dsts, Word _value, CbIdx _cbidx)
